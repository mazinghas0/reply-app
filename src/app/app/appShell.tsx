...
export default function AppShell({ initialAuth }: AppShellProps) {
  const [mode, setMode] = useState<AppMode>("generate");
  const [inputMessage, setInputMessage] = useState("");
  const [remaining, setRemaining] = useState<number | null>(initialAuth.credits);
  ...
  const [resetAt, setResetAt] = useState<string | null>(initialAuth.resetAt);
  const [isAuthenticated, setIsAuthenticated] = useState(initialAuth.isAuthenticated);
  const [showWelcome, setShowWelcome] = useState(false);
  const [showReferral, setShowReferral] = useState(false);
  const [referralCode, setReferralCode] = useState(initialAuth.referralCode);
  const [maxInputLength, setMaxInputLength] = useState(initialAuth.maxInputLength);
  const [allowSonnet, setAllowSonnet] = useState(initialAuth.allowSonnet);
  const [plan, setPlan] = useState<string | null>(initialAuth.plan);
  const [monthlyCredits, setMonthlyCredits] = useState(initialAuth.monthlyCredits);
  ...
  useEffect(() => {
    ...
    // 로그인 유저의 신규 가입 웰컴 (SSR에서 이미 isAuthenticated=true)
    if (initialAuth.isAuthenticated && !localStorage.getItem("reply-welcome-done")) {
      setShowWelcome(true);
      localStorage.setItem("reply-welcome-done", "1");
    }

    // 포커스 복귀 시 크레딧 재검증 (SSR 초기값은 덮어쓰지 않도록 마운트 직후는 스킵)
    const refreshCredits = () => {
      fetch("/api/credits")
        .then((res) => res.json())
        .then((data) => {
          if (typeof data.credits === "number") setRemaining(data.credits);
          if (data.resetAt) setResetAt(data.resetAt);
          if (data.referralCode) setReferralCode(data.referralCode);
          if (typeof data.maxInputLength === "number") setMaxInputLength(data.maxInputLength);
          if (typeof data.allowSonnet === "boolean") setAllowSonnet(data.allowSonnet);
          if (data.plan) setPlan(data.plan);
          if (typeof data.monthlyCredits === "number") setMonthlyCredits(data.monthlyCredits);
    ...
    <div id="app-container" className="flex-1 flex flex-col bg-white dark:bg-slate-950 transition-colors duration-200">
      ...
      <main className="flex-1 flex flex-col items-center px-4 py-8 max-w-xl mx-auto w-full">
        ...
        <div data-tour="tour-tab-bar" className="w-full flex rounded-xl border border-slate-200 dark:border-slate-700 p-1 bg-slate-50 dark:bg-slate-900 transition-colors duration-200">
          ...
          <button onClick={() => setMode("refine")} className={tabClass(mode === "refine")}>말하기</button>
        ...
        <p className="w-full text-center text-xs text-slate-400 dark:text-slate-500 mt-1.5 mb-5">
          {mode === "generate"
            ...
            : mode === "refine"
            ? "하고 싶은 말을 톤에 맞는 메시지 3개로 만들어요"
            : "마지막 초안을 이어서 작성할 수 있어요"}
        </p>
            ...
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="text-xs text-teal-600 dark:text-teal-400 mt-1 leading-relaxed">
                  매달 10크레딧이 무료로 충전돼요. 답장 만들기, 검토, 말하기 모두 3크레딧이에요. 메시지를 붙여넣고 시작해 보세요!
                </p>
              </div>
